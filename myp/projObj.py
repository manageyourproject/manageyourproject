...
class projObj:
    def __init__(self, confObj, projName):
        ...
        self.projPath = os.path.join(self.projDir,self.names[0])
        self.projFile = os.path.join(self.projPath, self.names[-1]+'.json')
    ...
    def newProj(self, confObj):
        ...
        if len(self.names) > 1 and self.names[0]==self.names[-1]:
            raise click.ClickException('Can\'t have identically named project and subproject')
    ...
    def writeProj(self):
        with open(self.projFile, 'w') as fp:
            json.dump(self.projDat, fp, indent=4, separators=(',',':'))

    def readProj(self):
        if not os.path.isfile(self.projFile):
            raise click.ClickException('No project/subproject by that name')
        else:
    ...
    def newTask(self, taskName):
        ...
        if len(taskName) > 1 and taskName[0]==taskName[-1]:
            raise click.ClickException('Can\'t have identically named task and subtask')
        ...
        if taskName[-1] in self.projDat['tasks']:
            ...
            if len(taskName) > 1:
                self.projDat['tasks'][taskName[-1]]['parent'].append(taskName[0])
                self.projDat['tasks'][taskName[0]]['children'].append(taskName[-1])
            self.writeProj()

    def startTask(self, taskName):
        if taskName not in self.projDat['tasks']:
            if click.confirm('No task by that name exists. Would you like to create it?', abort=True):
                self.newTask(taskName)

        self.projDat['tasks'][taskName]['status'] = 'active'
        self.projDat['tasks'][taskName]['started'] = datetime.datetime.now(datetime.timezone.utc).isoformat()
        self.writeProj()

    def stopTask(self, taskName):
        if not self.projDat['tasks'][taskName]['started']:
            self.projDat['tasks'][taskName]['status']='in-progress'
            raise click.ClickException('That task isn\'t running')
        else:
            self.projDat['tasks'][taskName]['status']='in-progress'
            timeDuo = [self.projDat['tasks'][taskName]['started'],datetime.datetime.now(datetime.timezone.utc).isoformat()]
            self.projDat['tasks'][taskName]['sessions'].append(timeDuo)
            self.projDat['tasks'][taskName]['started']=''
            self.projDat['tasks'][taskName]['timeSpent']=self.projDat['tasks'][taskName]['timeSpent']+((datetime.datetime.fromisoformat(timeDuo[1])-datetime.datetime.fromisoformat(timeDuo[0])).total_seconds())
            self.writeProj()

    def finishTask(self, taskName):
        if 'finished' == self.projDat['tasks'][taskName]['status']:
            raise click.ClickException('That task is already finished')
        else:
            ...
            if self.projDat['tasks'][taskName]['children']:
                for i in self.projDat['tasks'][taskName]['children']:
                    self.finishTask(i)
